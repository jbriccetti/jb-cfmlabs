...
<cfcomponent output="false">
	...
	<cffunction name="send" access="public" returntype="struct" output="no" hint="Send an error notification to Airbrake">
		...
		<cfif arraylen(local.backtrace)>
			...
			<cfloop from="1" to="#ArrayLen(local.backtrace)#" index="local.line">
				<cfif isDefined("local.line.file") && isSimpleValue(local.line.file) && isDefined("local.line.line") and isSImpleValue(local.line.line)>
					<cfset local.xml.append('<line file="#xmlformat(local.line.file)#" number="#xmlformat(local.line.line)#"')>
				</cfif>
				<cfif isDefined("local.line.method") && isSimpleValue(local.line.method) && len(local.line.method)><cfset local.xml.append(' method="#xmlformat(local.line.method)#"')></cfif>
	...
	<cffunction name="build_backtrace" access="private" returntype="array" output="no" hint="Cleans up the context array and pulls out the information required for the backtrace">
		...
		<cfloop from="1" to="#ArrayLen(arguments.context)#" index="item">
			...
			<cfif isStruct(item) && structkeyexists(item, "line")><cfset line.line = item.line></cfif>
			<cfif isStruct(item) && structkeyexists(item, "template")><cfset line.file = item.template></cfif>
			<cfif isStruct(item) && structkeyexists(item, "raw_trace") AND refind("at cf.*?\$func([A-Z_-]+)\.runFunction", item.raw_trace)>
